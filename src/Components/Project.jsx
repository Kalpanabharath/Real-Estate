...
const Project = () => {
  const [currentIndex, setCurrecntInex] = useState(0);
  ...
  useEffect(() => {
    ...
    return () => window.addEventListener("resize", ubdateCardsToShow);
  }, []);

  const nextProject = () => {
    setCurrecntInex((prevIndex) => (prevIndex + 1) % projectsData.length);
  };
  const prevProject = () => {
    setCurrecntInex((prevIndex) =>
